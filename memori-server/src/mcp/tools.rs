...
pub fn list_tools() -> ListToolsResult {
    ListToolsResult {
        tools: vec![
            tool(TOOL_ASK, "Ask Memori-Vault and return answer, citations, evidence, and retrieval metrics.", schema(&["query"])),
            tool(TOOL_SEARCH, "Retrieve ranked source chunks without generating an answer.", schema(&["query"])),
            tool(TOOL_GET_SOURCE, "Read a source by file_path, chunk_id, or citation_index with a query.", schema(&[])),
            tool(TOOL_OPEN_SOURCE, "Open a local source file in the desktop environment.", schema(&["path"])),
            tool(TOOL_GET_VAULT_STATS, "Return vault document/chunk/graph statistics.", schema(&[])),
            tool(TOOL_GET_INDEXING_STATUS, "Return indexing phase, queue, and rebuild status.", schema(&[])),
            tool(TOOL_TRIGGER_REINDEX, "Trigger a full reindex task.", schema(&[])),
            tool(TOOL_PAUSE_INDEXING, "Pause background indexing.", schema(&[])),
            tool(TOOL_RESUME_INDEXING, "Resume background indexing.", schema(&[])),
            tool(TOOL_SET_INDEXING_MODE, "Set indexing mode/resource budget/schedule.", schema(&["indexing_mode", "resource_budget"])),
            tool(TOOL_SET_WATCH_ROOT, "Change the vault watch root and reload the engine.", schema(&["path"])),
            tool(TOOL_GET_MODEL_SETTINGS, "Return active local/remote model settings.", schema(&[])),
            tool(TOOL_SET_MODEL_SETTINGS, "Replace model settings and hot-reload runtime.", schema(&["active_provider", "local_profile", "remote_profile"])),
            tool(TOOL_VALIDATE_MODEL_SETUP, "Validate active model configuration.", schema(&[])),
            tool(TOOL_LIST_PROVIDER_MODELS, "List model candidates from provider/folder.", schema(&["provider", "endpoint"])),
            tool(TOOL_PROBE_MODEL_PROVIDER, "Probe a model provider endpoint.", schema(&["provider", "endpoint"])),
            tool(TOOL_PULL_MODEL, "Pull a local Ollama model; may be long-running.", schema(&["model", "provider", "endpoint"])),
            tool(TOOL_GET_GRAPH_CONTEXT, "Return graph context for a query or chunk ids.", schema(&[])),
            tool(TOOL_GET_GRAPH_NEIGHBORS, "Return 1-hop graph neighbors and source chunks for an entity id.", schema(&["entity_id"])),
            tool(TOOL_LIST_GRAPH_ENTITIES, "Search graph entities by id/name/label/description.", schema(&["query"])),
            tool(TOOL_GET_APP_SETTINGS, "Return app settings such as watch root and indexing config.", schema(&[])),
            tool(TOOL_GET_ENTERPRISE_POLICY, "Return enterprise egress/model policy.", schema(&[])),
            tool(TOOL_GET_RUNTIME_BASELINE, "Return runtime retrieval baseline diagnostics.", schema(&[])),
            tool(TOOL_RANK_SETTINGS, "Rank settings tabs for a natural-language settings query.", schema(&["query", "candidates"])),
        ],
    }
}
...
pub async fn call_tool(
    ...
    let result = match call.name.as_str() {
        ...
        TOOL_GET_MODEL_SETTINGS => Ok(json!(resolve_model_settings(&load_app_settings().map_err(JsonRpcError::internal_error)?))),
        ...
        TOOL_GET_ENTERPRISE_POLICY => Ok(json!(resolve_enterprise_policy(&load_app_settings().map_err(JsonRpcError::internal_error)?))),
        TOOL_GET_RUNTIME_BASELINE => get_runtime_baseline(state).await,
        TOOL_RANK_SETTINGS => rank_settings(state, call.arguments).await,
        _ => Err(JsonRpcError::method_not_found(format!("unknown tool: {}", call.name))),
    };
...
async fn get_source(state: ServerState, args: Option<JsonValue>) -> Result<JsonValue, JsonRpcError> {
    ...
    if let Some(chunk_id) = args.chunk_id {
        ...
        let Some(chunk) = store
            ...
            return Err(JsonRpcError::invalid_params(format!("chunk not found: {chunk_id}")));
        };
        let doc = store
    ...
    if let Some(index) = args.citation_index {
        ...
        if query.trim().is_empty() {
            return Err(JsonRpcError::invalid_params("query is required when using citation_index"));
        }
        let inspection = engine
            .retrieve_structured(query, None, Some(index.max(1)))
            .await
            .map_err(|err| JsonRpcError::internal_error(err.to_string()))?;
        let Some(citation) = inspection.citations.into_iter().find(|item| item.index == index) else {
            return Err(JsonRpcError::invalid_params(format!("citation not found: {index}")));
        };
    ...
    Err(JsonRpcError::invalid_params("one of file_path, chunk_id, or citation_index is required"))
}
...
async fn set_indexing_mode(state: ServerState, args: Option<JsonValue>) -> Result<JsonValue, JsonRpcError> {
...
async fn set_watch_root(state: ServerState, args: Option<JsonValue>) -> Result<JsonValue, JsonRpcError> {
    ...
    if !watch_root.is_dir() {
        return Err(JsonRpcError::invalid_params(format!("not a directory: {}", watch_root.display())));
    }
...
async fn set_model_settings(state: ServerState, args: Option<JsonValue>) -> Result<JsonValue, JsonRpcError> {
    let payload = parse_params::<ModelSettingsDto>(args)?;
    let mut settings = load_app_settings().map_err(JsonRpcError::internal_error)?;
    let normalized = normalize_model_settings_payload(payload).map_err(JsonRpcError::invalid_params)?;
    let policy = resolve_enterprise_policy(&settings);
    let active_runtime = resolve_active_runtime_settings(&normalized);
    validate_runtime_model_settings(&to_model_policy(&policy), &to_runtime_model_config(&active_runtime))
    ...
    replace_engine(&state.engine, &state.init_error, watch_root, "mcp_set_model_settings")
        .await
        .map_err(JsonRpcError::internal_error)?;
    Ok(json!(normalized))
}
...
async fn validate_model_setup() -> Result<JsonValue, JsonRpcError> {
    ...
    ).await.map_err(|err| JsonRpcError::internal_error(format!("{}: {}", err.code, err.message)))?;
...
async fn probe_model_provider(args: Option<JsonValue>) -> Result<JsonValue, JsonRpcError> {
    ...
    .await
    {
        Ok(models) => Ok(json!({ "reachable": true, "models": models.merged, "errors": [], "checked_provider": provider_to_string(provider) })),
        Err(err) => Ok(json!({ "reachable": false, "models": [], "errors": [{ "code": err.code, "message": err.message }], "checked_provider": provider_to_string(provider) })),
    }
}
...
async fn pull_model(args: Option<JsonValue>) -> Result<JsonValue, JsonRpcError> {
    ...
    if ModelProvider::from_value(&payload.provider) != ModelProvider::OllamaLocal {
        return Err(JsonRpcError::invalid_params("pull_model only supports ollama_local"));
    }
...
async fn get_graph_context(state: ServerState, args: Option<JsonValue>) -> Result<JsonValue, JsonRpcError> {
    let args = parse_params::<GraphContextArgs>(args)?;
    let engine = engine_from_state(&state).await?;
    let chunk_ids = if args.chunk_ids.is_empty() {
        let query = args.query.as_deref().unwrap_or_default();
        if query.trim().is_empty() {
            return Err(JsonRpcError::invalid_params("query or chunk_ids is required"));
        }
        let inspection = engine
...
async fn get_graph_neighbors(state: ServerState, args: Option<JsonValue>) -> Result<JsonValue, JsonRpcError> {
...
async fn list_graph_entities(state: ServerState, args: Option<JsonValue>) -> Result<JsonValue, JsonRpcError> {
...
async fn rank_settings(state: ServerState, args: Option<JsonValue>) -> Result<JsonValue, JsonRpcError> {
...
fn schema(required: &[&str]) -> JsonValue {
    ...
    for key in required {
        let prop = match *key {
            ...
            "top_k" | "topK" => json!({ "type": "integer", "description": "Number of top results" }),
            "lang" => json!({ "type": "string", "description": "Language code (e.g. zh-CN, en-US)" }),
            "scope_paths" | "scopePaths" => json!({ "type": "array", "items": { "type": "string" }, "description": "Limit search to these paths" }),
            "chunk_ids" => json!({ "type": "array", "items": { "type": "integer" }, "description": "List of chunk identifiers" }),
            "model" => json!({ "type": "string", "description": "Model name to pull" }),
            "provider" => json!({ "type": "string", "enum": ["ollama_local", "openai_compatible"], "description": "Model provider" }),
            ...
            "indexing_mode" => json!({ "type": "string", "enum": ["continuous", "manual", "scheduled"], "description": "Indexing strategy" }),
            "resource_budget" => json!({ "type": "string", "enum": ["low", "balanced", "fast"], "description": "Resource usage level" }),
            "schedule_start" => json!({ "type": "string", "description": "Schedule start time (HH:MM)" }),
            "schedule_end" => json!({ "type": "string", "description": "Schedule end time (HH:MM)" }),
            "active_provider" => json!({ "type": "string", "enum": ["ollama_local", "openai_compatible"], "description": "Active provider" }),
            "local_profile" => json!({ "type": "object", "description": "Local provider profile" }),
            "remote_profile" => json!({ "type": "object", "description": "Remote provider profile" }),
            "candidates" => json!({ "type": "array", "items": { "type": "string" }, "description": "Candidate tab names to rank" }),
            _ => json!({ "type": "string" }),
        };
