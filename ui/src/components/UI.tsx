  ...
  return (
    <button
      ...
      className={`relative h-5 w-10 rounded-full transition-colors duration-300 ${
        checked
          ? "border border-[#58a6ff] bg-[#1f6feb]/30"
    ...
    >
      <span
        className={`absolute left-[1px] top-[1px] h-4 w-4 rounded-full transition-transform duration-300 ${
          checked
            ? "translate-x-5 bg-[#58a6ff] shadow-[0_0_10px_#58a6ff]"
            : "bg-[#8b949e]"
        }`}
      />
    </button>
...
export function CyberInput({ value, onChange, placeholder, className }: CyberInputProps) {
  return (
    <input
      ...
      className={`w-full rounded-lg border border-[#30363d] bg-[#0d1117]/50 px-3 py-2 text-sm text-[#c9d1d9] placeholder:text-[#6e7681] transition-all focus:outline-none focus:ring-1 focus:ring-[#58a6ff] ${className ?? ""}`}
    />
  );
}
